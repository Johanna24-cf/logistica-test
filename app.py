# =========================================================
# SISTEMA LOGÍSTICO CARCASAS - VERSIÓN FINAL CERTIFICADA
# =========================================================
...
# 2. ESTILOS CSS (Mantenidos para diseño de tarjetas)
st.markdown("""
    <style>
...
# 4. FUNCIONES DE ACTUALIZACIÓN (ANTI-ERROR 429 + LÓGICA X1-X9)
...
# 6. VISTAS ORIGINALES (Mantenidas según imagen)
    ...
    with tab_recep:
        ...
        if not df_recepcion.empty:
            ...
            with col2:
                ...
                if not df_alm.empty: st.dataframe(df_alm.groupby(["IMPORTACION", "DESTINO"]).size().reset_index(name="BULTOS"), use_container_width=True, hide_index=True)
            with col3:
                ...
                if not df_prog.empty:
                    st.dataframe(df_prog.groupby(["FECHA ENTREGA", "IMPORTACION"]).size().reset_index(name="BULTOS"), use_container_width=True, hide_index=True)
                ...
                with st.form("f_arribo"):
                    ...
                    if st.form_submit_button("Confirmar Arribo Masivo"):
                        if update_consolidado_arribo(d_sel, f_sel):
                            st.success("¡Operación Exitosa!"); st.cache_data.clear(); st.rerun()
        with o2:
